...
app.get("/api/get_ticket", async (req, res) => {
  ...
  if (esFechaCreacionOPublicidad == "true") {
    ...
    filtrado = filtrado.where(
      "fechaCreacion",
      ">=",
      new Date(fechaCreacionStart)
    );
    filtrado = filtrado.where(
      "fechaCreacion",
      "<=",
      new Date(fechaCreacionEnd)
    );
  } else {
    ...
    filtrado = filtrado.where(
      "fechaPublicacion",
      ">=",
      new Date(fechaPublicacionStart)
    );
  ...
  if (valorCompraEnd != 0 || valorCompraStart != 0) {
    ticketData = ticketData.filter(function (value) {
      return (
        value.valorCompra > valorCompraStart &&
        value.valorCompra < valorCompraEnd
      );
    });
  }
...
app.post("/api/borrar_categoria/:id", async (req, res) => {
  console.log(
    "--------------Se ejecuto la llamada borrarCategorias-----------"
  );
...
app.post("/api/actualizar_categoria/:id", async (req, res) => {
  console.log(
    "--------------Se ejecuto la llamada actualizarCategorias-----------"
  );
